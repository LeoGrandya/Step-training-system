  ...
  <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-lg glow-border-sky">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3">
      <div class="bg-sky-50 rounded-lg border border-slate-200/60 p-2">
        <p class="text-sm text-slate-400 mb-1 font-medium">步伐周期甘特图（1号步伐）</p>
        <div v-if="hasGantt" ref="ganttRef" class="w-full h-48"></div>
        <ReportEmptyState v-else text="暂无周期数据" />
      </div>
      <div class="bg-sky-50 rounded-lg border border-slate-200/60 p-2">
        <p class="text-sm text-slate-400 mb-1 font-medium">重心相平面图（移动/制动动态）</p>
        <div v-if="hasPhasePlane" ref="phaseRef" class="w-full h-48"></div>
        <ReportEmptyState v-else text="暂无相平面数据" />
      </div>
      <div class="bg-sky-50 rounded-lg border border-slate-200/60 p-2">
        <p class="text-sm text-slate-400 mb-1 font-medium">重心合速度曲线及阶段划分</p>
        <div v-if="hasSpeed" ref="speedRef" class="w-full h-48"></div>
        <ReportEmptyState v-else text="暂无速度数据" />
      </div>
    </div>
  </div>
</template>
...
const hasGantt = computed(() => !!props.gantt)
const hasPhasePlane = computed(() => !!props.phasePlane)
const hasSpeed = computed(() => !!props.speedOption)
const hasAnyData = computed(() => hasGantt.value || hasPhasePlane.value || hasSpeed.value)
...
function renderCharts() {
  ...
  if (ganttRef.value) {
    ...
    if (props.gantt) c.setOption(props.gantt, true)
    else if (isDev) c.setOption(mockGantt(), true)
    charts.push(c)
  }
  if (phaseRef.value) {
    const c = echarts.init(phaseRef.value)
    if (props.phasePlane) c.setOption(props.phasePlane, true)
    else if (isDev) c.setOption(mockPhasePlane(), true)
    charts.push(c)
  }
  if (speedRef.value) {
    const c = echarts.init(speedRef.value)
    if (props.speedOption) c.setOption(props.speedOption, true)
    else if (isDev) c.setOption(mockSpeed(), true)
    charts.push(c)
  }
}
